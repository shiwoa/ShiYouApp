...
final class ZLXMLParser {
    ...
    private final BufferedInputStream bufferedIS;//用于读取加密过的html文件
    private final ZLXMLReader myXMLReader;
    private final boolean myProcessNamespaces;
    private final char[] myBuffer;
    private final byte[] myByteBuffer;//用于读取加密过的html文件
    private final byte[] myDecrBuffer;//用于存放解密过的字节
    ...
    public ZLXMLParser(ZLXMLReader xmlReader, InputStream stream, int bufferSize, String type) throws IOException {
        myXMLReader = xmlReader;
        myProcessNamespaces = xmlReader.processNamespaces();
        final char[] buffer = getBuffer(bufferSize * 2);
        myBuffer = buffer;
        myByteBuffer = new byte[bufferSize];
        myDecrBuffer = new byte[bufferSize];

        myStreamReader = null;
        bufferedIS = new BufferedInputStream(stream);
    }
    ...
    void doIt(String type) throws IOException {
        final ZLXMLReader xmlReader = myXMLReader;
        final HashMap<String, char[]> entityMap = getDTDMap(xmlReader.externalDTDs());
        xmlReader.collectExternalEntities(entityMap);
        final BufferedInputStream bis = bufferedIS;
        final boolean processNamespaces = myProcessNamespaces;
        HashMap<String, String> oldNamespaceMap = processNamespaces ? new HashMap<String, String>() : null;
        HashMap<String, String> currentNamespaceMap = null;
        final ArrayList<HashMap<String, String>> namespaceMapStack = new ArrayList<HashMap<String, String>>();
        byte[] origBuffer = myByteBuffer;
        byte[] decrBuffer = myDecrBuffer;
        char[] buffer = myBuffer;
        final ZLMutableString tagName = myTagName;
        final ZLMutableString cData = myCData;
        final ZLMutableString attributeName = myAttributeName;
        ...
        byte[] key = DecryptionUtil.getKey();

        while (true) {
            int byteCount = bis.read(origBuffer);
            if (byteCount <= 0) {
                bis.close();
                return;
            }
            for (int i = 0; i < byteCount; i++) {
                decrBuffer[i] = (byte) (origBuffer[i] ^ key[i % key.length]);
            }
            byte[] bytes = new byte[byteCount];
            System.arraycopy(decrBuffer, 0, bytes, 0, byteCount);
            Charset charset = Charset.forName("UTF-8");
            char[] decryptedChars = charset.decode(ByteBuffer.wrap(bytes)).array();
            System.arraycopy(decryptedChars, 0, buffer, 0, decryptedChars.length);

            int count = decryptedChars.length;
            int startPosition = 0;
            if (count < buffer.length) {
                //buffer = ZLArrayUtils.createCopy(buffer, count, count);
                startPosition = buffer.length - count;
