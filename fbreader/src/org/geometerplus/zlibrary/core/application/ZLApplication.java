...
public abstract class ZLApplication {
    ...
    //���Լ�дһ��
    private ArrayList<PopupPanel> myActivePopups = new ArrayList<PopupPanel>();
...
/*		if (myActivePopup != null) {
        ...
        for (PopupPanel pop : myActivePopups) {
            pop.hide_();
        }
        FBReaderApp fbReader = (FBReaderApp) FBReaderApp.Instance();
        ZLTextView view = fbReader.getTextView();
        view.selectShow = false;
        myActivePopups.clear();

    }

    public final void showPopup(String id) {
        hideActivePopup();
        PopupPanel myActivePopup = myPopups.get(id);
        if (myActivePopup != null) {
            myActivePopup.show_();
            myActivePopups.add(myActivePopup);
        }
    }

    public final void showAllPopup() {
        for (PopupPanel pop : myActivePopups) {
            pop.show_();
        }
    }
    ...
    public final ArrayList<PopupPanel> getActivePopup() {
        return myActivePopups;
    }

    public final boolean hasActivePopup() {
        if (myActivePopups == null || myActivePopups.size() == 0) {
            return false;
        } else {
            return true;
        }
