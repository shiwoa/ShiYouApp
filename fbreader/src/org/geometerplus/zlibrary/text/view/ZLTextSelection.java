...
class ZLTextSelection implements ZLTextAbstractHighlighting {
    ...
    public boolean change() {
        ...
        stop();
        // 置空选择箭头就消失同时选中文字状态也被清除、如果注释掉这两句则选择箭头和文字都存在
        // 怎么实现清除选择箭头和文字背景、然后在选中文字下划线？
        // 解决？将cursor状态null，即清除了箭头，同时又保存了文字选择状态。对这样就可以了。接下来应该就是更换文字下线颜色
        myLeftMostRegionSoul = null;
        myRightMostRegionSoul = null;
        myCursorInMovement = ZLTextSelectionCursor.None;
        return true;
    ...
    boolean start(int x, int y) {
        ...
        final ZLTextRegion region = myView
                .findRegion(x, y, ZLTextView.MAX_SELECTION_DISTANCE,
    ...
    void expandTo(int x, int y) {
        ...
        if (firstArea != null && y < firstArea.YStart) {
            ...
            if (myScroller == null) {
                ...
                + ZLTextSelectionCursor.getAccent() / 2 > lastArea.YEnd) {
            if (myScroller != null && !myScroller.scrollsForward()) {
                myScroller.stop();
                myScroller = null;
            }
        ...
        if (myCursorInMovement == ZLTextSelectionCursor.Right) {
            if (hasAPartAfterPage(myView.myCurrentPage)) {
                myView.scrollPage(true, ZLTextView.ScrollingMode.SCROLL_LINES,
                        1);
            ...
            if (hasAPartBeforePage(myView.myCurrentPage)) {
                myView.scrollPage(false, ZLTextView.ScrollingMode.SCROLL_LINES,
                        1);
                myView.Application.getViewWidget().reset();
                myView.preparePaintInfo();
            }
    ...
    boolean isAreaSelected(ZLTextElementArea area) {
        return !isEmpty() && myLeftMostRegionSoul.compareTo(area) <= 0
    ...
    public ZLTextPosition getStartPosition() {
        ...
        return new ZLTextFixedPosition(myLeftMostRegionSoul.ParagraphIndex,
                myLeftMostRegionSoul.StartElementIndex, 0);
    }
    ...
    public ZLTextPosition getEndPosition() {
        ...
        return new ZLTextFixedPosition(myRightMostRegionSoul.ParagraphIndex,
                myRightMostRegionSoul.EndElementIndex, 0);
    }
    ...
    boolean hasAPartBeforePage(ZLTextPage page) {
        ...
        final ZLTextElementArea firstPageArea = page.TextElementMap
                .getFirstArea();
    ...
    boolean hasAPartAfterPage(ZLTextPage page) {
        ...
        final ZLTextElementArea lastPageArea = page.TextElementMap
                .getLastArea();
    ...
    private class Scroller implements Runnable {
        ...
        public void run() {
            myView.scrollPage(myScrollForward,
                    ZLTextView.ScrollingMode.SCROLL_LINES, 1);
            myView.preparePaintInfo();
            expandTo(myX, myY);
            myView.Application.getViewWidget().reset();
