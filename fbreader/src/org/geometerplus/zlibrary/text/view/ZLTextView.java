...
public abstract class ZLTextView extends ZLTextViewBase {
    ...
    private static final double FRONTCOVER_Y_MARGIN_RATE = 0.05;
    ...
    public boolean selectShow = false;
    ...
    public synchronized int search(final String text, boolean ignoreCase,
        ...
        if (FBReader.probationRate > 0) {
            endIndex = FBReader.probationRate * paragraphsNumber / 100;
        } else {
            endIndex = paragraphsNumber;
        }
        if (thisSectionOnly) {
            // TODO: implement
        }
        ...
        /*
         * myPreviousPage.reset(); myNextPage.reset(); if
		 * (!myCurrentPage.StartCursor.isNull()) { rebuildPaintInfo(); if (count
		 * > 0) { ZLTextMark mark = myCurrentPage.StartCursor.getMark();
		 * gotoMark(wholeText ? (backward ? myModel.getLastMark() :
		 * myModel.getFirstMark()) : (backward ? myModel .getPreviousMark(mark)
		 * : myModel.getNextMark(mark))); } Application.getViewWidget().reset();
		 * Application.getViewWidget().repaint(); }
		 */
        return count;
    }

    public void gotoSearchItem(int index) {
        myPreviousPage.reset();
        myNextPage.reset();
        if (!myCurrentPage.StartCursor.isNull()) {
            rebuildPaintInfo();
            gotoMark(myModel.getMarksOrSavedMarks().get(index));
            Application.getViewWidget().reset();
            Application.getViewWidget().repaint();
        }
    }
    ...
    public final synchronized PagePosition pagePosition() {
        ...
        // 保存阅读进度
        try {
            Context context = FBReaderApplication.context;
...
//            values.put("book_progress", nt.format(progress) + "");
//            contentResolver.update(
//                    Uri.parse("content://com.eputai.ebook.provider.MyProvider"),
//                    values, "bookid=?", new String[]{FBReader.bookID});
        } catch (Exception e) {
            e.printStackTrace();
        }
        if (total > 3) {
    ...
    private void drawBackgroung(ZLTextAbstractHighlighting highligting,
        ...
        if (!highligting.isEmpty() && from != to) {
            ...
            if (selectionStartArea != null && selectionEndArea != null
                ...
                color2 = Library.Instance().getSelectionColor(
                        reader.Model.Book.getId());
                ZLColor line_color = null;
                if (!StringUtils.isEmpty(color2)) {
                    if (color2.equals("1")) {
                        line_color = getSelectionYellowLineOption();
                    } else if (color2.equals("2")) {
                        line_color = getSelectionGreenLineOption();
                    } else if (color2.equals("3")) {
                        line_color = getSelectionBlueLineOption();
                    } else if (color2.equals("4")) {
                        line_color = getSelectionPurpleLineOption();
                    }
                } else {
                    color2 = "1";
                    line_color = getSelectionYellowLineOption();
                }
                myContext.setLineColor(line_color);
                myContext.setLineWidth(3);
                myContext.drawLine(left, bottom, right, bottom);
            }
        }
    }

    private void drawLine(ZLTextPage page, ZLTextLineInfo info, int from,
                          int to, int y) {
        if (from == to) {
            return;
        }
        int paragraphIndex = info.ParagraphCursor.Index;
        int realStartElementIndex = info.RealStartElementIndex;
        int endIndex = info.EndElementIndex;
        FBReaderApp fbReader = (FBReaderApp) FBReaderApp.Instance();
        long id = fbReader.Model.Book.getId();
        ArrayList<BookSelection> bookSelectionList = Library.Instance()
                .getBookSelection(id, paragraphIndex, realStartElementIndex,
                        endIndex);
        final int top = y + 1;
        int left, right, bottom = y + info.Height + info.Descent;
        if (bookSelectionList != null && bookSelectionList.size() != 0) {
            try {
                for (int i = 0; i < bookSelectionList.size(); i++) {
                    ...
                    // 开始位置
                    int startParagraphIndex = bookSelection.getParagraphIndex();
                    int endParagraphIndex = bookSelection
                            .getMyEndParagraphIndex();
                    if (startParagraphIndex == paragraphIndex) {
                        if (realStartElementIndex >= bookSelection.ElementIndex
                                && endIndex <= bookSelection.getMyIndex()) {
                            ZLTextElementArea fromArea = page.TextElementMap
                                    .get(from);
                            ZLTextElementArea toArea = page.TextElementMap
                                    .get(to - 1);
                            left = fromArea.XStart;
                            right = toArea.XEnd;
                        } else {
                            // 文字在当前行内 数据库中存取的当前位置减去这一行的开始位置
                            int space = bookSelection.ElementIndex
                                    - realStartElementIndex;
                            ZLTextElementArea fromArea;
                            if (space < 0) {
                                fromArea = page.TextElementMap.get(from);
                            } else {
                                fromArea = page.TextElementMap
                                        .get(from + space);
                            }
                            // 这一行的结束位置减去选中文字的结束位置
                            // String replaceAll =
                            // bookSelection.getMyText().replaceAll("(?i)[^a-zA-Z0-9\u4E00-\u9FA5]",
                            // "");
                            // int length = bookSelection.ElementIndex +
                            // replaceAll.length();
                            int endSpace = endIndex
                                    - bookSelection.getMyIndex();
                            ZLTextElementArea toArea;
                            if (endSpace < 0) {
                                toArea = page.TextElementMap.get(to - 1);
                            } else {
                                toArea = page.TextElementMap.get(to - endSpace
                                        - 1);
                            }
                            left = fromArea.XStart;
                            right = toArea.XEnd;
                        ...
                        if (paragraphIndex < endParagraphIndex) {
                            ...
                            // 在夸段落结束位置前全部划线
                            if (endIndex <= bookSelection
                                    .getMyEndElementIndex()) {
                                ZLTextElementArea fromArea = page.TextElementMap
                                        .get(from);
                                ZLTextElementArea toArea = page.TextElementMap
                                        .get(to - 1);
                                ...
                                fromArea = page.TextElementMap.get(from);
                                int endSpace = endIndex
                                        - bookSelection.getMyEndElementIndex();
                                ZLTextElementArea toArea;
                                if (endSpace < 0) {
                                    toArea = page.TextElementMap.get(to);
                                } else {
                                    toArea = page.TextElementMap.get(to
                                            - endSpace);
                                }
                    ...
                    bottom += info.VSpaceAfter;
                    ZLColor line_color = null;
                    String color = bookSelection.getColor();
        ...
        for (int wordIndex = info.RealStartElementIndex; wordIndex != endElementIndex
            ...
            if (element == area.Element) {
                ...
                if (element instanceof ZLTextWord) {
                    ...
                    // getSelectedForegroundColor():避免划线的时候字体颜色变化

                    drawWord(areaX, areaY, (ZLTextWord) element, charIndex, -1,
                            false, getTextColor(getTextStyle().Hyperlink));
                } else if (element instanceof ZLTextImageElement) {
                    final ZLTextImageElement imageElement = (ZLTextImageElement) element;
                    if (imageElement.Id.endsWith("frontcover.jpg")
                            || imageElement.Id.endsWith("Frontcover.jpg")) {
                        context.drawImage(
                                context.getWidth(),
                                imageElement.ImageData,
                                new ZLPaintContext.Size(
                                        getTextAreaWidth(),
                                        (int) (getTextAreaHeight() * (1 - FRONTCOVER_Y_MARGIN_RATE))),
                                ZLPaintContext.ScalingType.FitMaximum,
                                FRONTCOVER_Y_MARGIN_RATE);
                        Log.d("cover", "draw frontcover");
                    } else {
                        context.drawImage(
                                areaX,
                                areaY,
                                imageElement.ImageData,
                                getTextAreaSize(),
                                imageElement.IsCover ? ZLPaintContext.ScalingType.FitMaximum
    ...
    public boolean onFingerRelease(int x, int y) {
        ...
        if (cursor != ZLTextSelectionCursor.None) {
            ...
            ZLTextPosition selectionEndPosition = fbview
                    .getSelectionEndPosition();
            int endParagraphIndex = selectionEndPosition.getParagraphIndex();
            int endElementIndex = selectionEndPosition.getElementIndex();
            // 保存书摘文字
            new BookSelection(fbReader.Model.Book, FBReader.bookID,
                    fbview.getSelectionStartPosition(), endParagraphIndex,
                    endElementIndex, text, color2).save();
            return false;
        }
        return false;
    }
    ...
    public boolean onFingerSingleTap(int x, int y) {
        ...
        // if (region != null) {
        // int paragraphIndex = region.getFirstArea().getParagraphIndex();
        // int elementIndex = region.getFirstArea().getElementIndex();
        // int elementIndex2 = region.getLastArea().getElementIndex();
        // FBReaderApp fbReader = (FBReaderApp) FBReaderApp.Instance();
        // long id = fbReader.Model.Book.getId();
        // ArrayList<BookSelection> bookSelectionList = Library.Instance()
        // .getBookSelection(id, paragraphIndex, elementIndex,
        // elementIndex2);
        // if (bookSelectionList != null && bookSelectionList.size() != 0) {
        // BookSelection bookSelection = bookSelectionList.get(0);
        // int elementIndex3 = bookSelection.getElementIndex();
        // int myEndElementIndex = bookSelection.getMyEndElementIndex();
        // fbReader.BookTextView.initSelection(x, y);
        // final ZLTextSelectionCursor cursor = findSelectionCursor(x,
        // y);
        // ZLTextElementArea fromArea = myCurrentPage.TextElementMap
        // .get(elementIndex3);
        // ZLTextElementArea elementByCoordinates = getElementByCoordinates(x,
        // y);
        // // ZLTextElementArea fromArea_em = myCurrentPage.TextElementMap
        // // .get(elementIndex);
        // // ZLTextElementArea toArea = myCurrentPage.TextElementMap
        // // .get(myEndElementIndex - 1);
        // // fbReader.BookTextView.moveSelectionCursorTo(cursor, 222, 677);
        // fbReader.runAction(ActionCode.SELECTION_SHOW_PANEL);
        // return true;
        // }
        // }
        return super.onFingerSingleTap(x, y);
    }
    ...
    public boolean onFingerReleaseAfterLongPress(int x, int y) {
        ...
        return super.onFingerReleaseAfterLongPress(x, y);
    }
    ...
    public void changeSelectionColor(ZLColor lineOption) {
