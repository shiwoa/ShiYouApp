...
public final class Library extends AbstractLibrary {
    ...
    public Bookmark getBookmark(long bookId, int paragraphIndex) {
        return BooksDatabase.Instance().getBookmark(bookId, paragraphIndex, true);
    }

    public List<Integer> getBookmarkParagraphIndexes(long bookId) {
        return BooksDatabase.Instance().getBookmarkParagraphIndexes(bookId);
    }


    public ArrayList<BookSelection> getBookSelection(long bookId, int paragraphIndex, int elementIndex, int endIndex) {
        return BooksDatabase.Instance().getBookSelection(bookId, paragraphIndex, elementIndex, endIndex);
    }

    public String getSelectionColor(long bookId) {
        return BooksDatabase.Instance().getBookSelectionColor(bookId);
    }

    @Override
    public List<Booknote> invisibleBooknotes(Book book) {
        final List<Booknote> list = BooksDatabase.Instance().loadBooknotes(book.getId(), false);
        Collections.sort(list, new Booknote.ByTimeComparator());
        return list;
    }

    @Override
    public List<Booknote> allBooknotes() {
        return BooksDatabase.Instance().loadAllVisibleBooknotes();
    }
