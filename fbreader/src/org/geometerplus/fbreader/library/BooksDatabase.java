...
public abstract class BooksDatabase {
    ...
    protected Booknote createBooknote(long id, long bookId, String bookTitle, String text, Date creationDate, Date modificationDate, Date accessDate, int accessCounter, String modelId, int paragraphIndex, int wordIndex, int charIndex, boolean isVisible) {
        return new Booknote(id, bookId, bookTitle, text, creationDate, modificationDate, accessDate, accessCounter, modelId, paragraphIndex, wordIndex, charIndex, isVisible);
    }

    protected abstract List<Booknote> loadBooknotes(long bookId, boolean isVisible);

    protected abstract List<Booknote> loadAllVisibleBooknotes();

    protected abstract long saveBooknote(Booknote bookmark);

    protected abstract void deleteBooknote(Booknote bookmark);


    protected BookSelection createBookSelection(long id, long bookId, String text, Date creationDate, Date modificationDate, int paragraphIndex, int wordIndex, int charIndex, int endParagraphIndex, int endElementIndex, int endIndex, String color) {
        return new BookSelection(id, bookId, text, creationDate, modificationDate, paragraphIndex, wordIndex, charIndex, endParagraphIndex, endElementIndex, endIndex, color);
    }

    //保存书摘文字
    protected abstract long saveSelection(BookSelection bookSelection);

    protected abstract void updateSelection(BookSelection bookSelection);

    protected abstract void deleteSelection(BookSelection bookSelection);

    protected abstract ArrayList<BookSelection> getBookSelection(long bookId, int paragraphIndex, int elementIndex, int endIndex);

    protected abstract String getBookSelectionColor(long bookId);
    ...
    protected abstract Bookmark getBookmark(long bookId, int paragraphIndex, boolean isVisible);

    protected abstract List<Integer> getBookmarkParagraphIndexes(long bookId);
