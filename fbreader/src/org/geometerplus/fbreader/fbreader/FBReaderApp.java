...
public final class FBReaderApp extends ZLApplication {
    ...
    public FBReaderApp() {
        ...
        //行间距
        addAction(ActionCode.SPACE_SMALL, new ChangeLineSpaceAction(this, 10));
        addAction(ActionCode.SPACE_MIDDLE, new ChangeLineSpaceAction(this, 13));
        addAction(ActionCode.SPACE_BIG, new ChangeLineSpaceAction(this, 20));
        addAction(ActionCode.SPACE_ORIGINAL, new ChangeLineSpaceAction(this, 16));
...
//		addAction(ActionCode.TURN_PAGE_BACK, new TurnPageAction(this, false));
    ...
    public void openBookNote(final Book book, final Booknote booknote, final Runnable postAction) {
        if (book != null || Model == null) {
            runWithMessage("loadingBook", new Runnable() {
                public void run() {
                    openBookInternalNote(book, booknote);
                }
            }, postAction);
        }
    }
    ...
    public void tryOpenFootnote(String id, Context context) {
        if (Model != null) {
            ...
            if (label != null) {
                if (label.ModelId == null) {
                    if (getTextView() == BookTextView) {
                        int paragraphsNumber = this.Model.getTextModel().getParagraphsNumber();
                        if (FBReader.probationRate > 0) {
                            int limitParagraphsNumber = FBReader.probationRate * paragraphsNumber / 100;
                            if (label.ParagraphIndex > limitParagraphsNumber) {
                                int pageLimit = this.getPageUpperLimit();
...
//			book = Library.Instance().getRecentBook();
    ...
    synchronized void openBookInternalNote(Book book, Booknote bookmark) {
        if (book == null) {
            //打开书籍失败了 打开最近打开的一本书
//			book = Library.Instance().getRecentBook();
			/*if (book == null || !book.File.exists()) {
    ...
    public void gotoBooknote(Booknote bookmark) {
        final String modelId = bookmark.ModelId;
        if (modelId == null) {
            addInvisibleBookmark();
            BookTextView.gotoPosition(bookmark);
...
//            }
    ...
    private synchronized void updateInvisibleBooknotesList(Booknote b) {
        if (Model != null && Model.Book != null && b != null) {
            for (Booknote bm : Library.Instance().invisibleBooknotes(Model.Book)) {
                if (b.equals(bm)) {
                    bm.delete();
                }
            }
            b.save();
//            final List<Booknote> booknotes = Library.Instance().invisibleBooknotes(Model.Book);
//            for (int i = 3; i < booknotes.size(); ++i) {
//                booknotes.get(i).delete();
//            }
        }
    }
    ...
    public void addInvisibleBooknote(ZLTextWordCursor cursor, String infor) {
        if (cursor != null && Model != null && Model.Book != null && getTextView() == BookTextView) {
            updateInvisibleBooknotesList(new Booknote(
                    Model.Book,
                    getTextView().getModel().getId(),
                    cursor,
                    6,
    ...
    public Booknote addBooknote(int maxLength, boolean visible) {
        final FBView view = getTextView();
        final ZLTextWordCursor cursor = view.getStartCursor();
    ...
    public int getPageUpperLimit() {
        return getTextView().pagePosition().Total * FBReader.probationRate / 100;
    }
