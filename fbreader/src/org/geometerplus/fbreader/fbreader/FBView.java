...
public final class FBView extends ZLTextView {
    ...
    //头部和底部的字体大小
    public static int footerAndHeaderSize = 30;
    private FBReaderApp myReader;
    private long lastTime;// 用于标记上次切换菜单的时刻
    ...
    private TapZoneMap getZoneMap() {
        ...
        if ("".equals(id)) {
            id = ScrollingPreferences.Instance().HorizontalOption.getValue() ? "right_to_left"
                    : "up";
        }
    ...
    public boolean onFingerSingleTap(int x, int y) {
        ...
        int height = getHeaderArea().getHeight();// 页眉导致的y坐标偏差
        final ZLTextRegion region = findRegion(x, y - height,
                MAX_SELECTION_DISTANCE, ZLTextRegion.HyperlinkFilter);
        if (region != null) {
            selectRegion(region);
        ...
        TapZoneMap zoneMap = getZoneMap();
        String actionID = zoneMap.getActionByCoordinates(x, y, myContext
                        .getWidth(), myContext.getHeight(),
                isDoubleTapSupported() ? TapZoneMap.Tap.singleNotDoubleTap
                        : TapZoneMap.Tap.singleTap);

        if (actionID == null) {
            if (myReader.hasActivePopup()) {
                myReader.hideActivePopup();
                // 增加去掉选择箭头
                myReader.getTextView().clearSelection();
            } else {
                long currentTimeMillis = System.currentTimeMillis();
                if (currentTimeMillis - lastTime > 500) {
                    myReader.runAction(ActionCode.TOGGLE_MENU);
                    lastTime = currentTimeMillis;
                }
            }
        }
        // 将显示快速翻页的行为改为向后翻页
        else if (ActionCode.SHOW_NAVIGATION.equals(actionID)) {
            myReader.runAction(ActionCode.TURN_PAGE_BACK);
        }
        // 将显示选项菜单的行为改为向前翻页
        else if (ActionCode.SHOW_MENU.equals(actionID)) {
            myReader.runAction(ActionCode.TURN_PAGE_FORWARD);
        } else {
            myReader.runAction(actionID, x, y);
        }
        /*
		 * //���κε���¼������ó������� if(selectShow){ Log.d("mouee", "���ز˵�");
		 * myReader.hideActivePopup(); }else{ Log.d("mouee", "��ʾ�˵�");
		 * myReader.runAction(ActionCode.SHOW_NAVIGATION, x, y); }
		 * this.clearSelection();
		 *
		 * //myReader.runAction("menu", x, y);
		 */
    ...
    public ZLColor getSelectionYellowLineOption() {
        return myReader.getColorProfile().SelectionYellowLineOption.getValue();
    }

    @Override
    public ZLColor getSelectionGreenLineOption() {
        return myReader.getColorProfile().SelectionGreenLineOption.getValue();
    }

    @Override
    public ZLColor getSelectionBlueLineOption() {
        return myReader.getColorProfile().SelectionBlueLineOption.getValue();
    }

    @Override
    public ZLColor getSelectionPurpleLineOption() {
        return myReader.getColorProfile().SelectionPurpleLineOption.getValue();
    }
    ...
    private class Footer implements FooterArea {
        ...
        public synchronized void paint(ZLPaintContext context) {
            ...
            final ZLColor fgColor = new ZLColor(170, 170, 170);
            // final ZLColor fgColor = new ZLColor(255, 0, 0);
            ...
            // 修改字体设置
			/*
			 * context.setFont( reader.FooterFontOption.getValue(), height <= 10
			 * ? height + 3 : height + 1, height > 10, false, false, false );
			 */
            context.setFont(reader.FooterFontOption.getValue(), footerAndHeaderSize, false,
                    false, false, false);
            ...
            // 不显示电池电量和当前时间
			/*
			 * if (reader.FooterShowBatteryOption.getValue()) { if
			 * (info.length() > 0) { info.append(" "); }
			 * info.append(reader.getBatteryLevel()); info.append("%"); } if
			 * (reader.FooterShowClockOption.getValue()) { if (info.length() >
			 * 0) { info.append(" "); }
            ...
            // 绘制页码信息
            context.drawString(right - infoWidth, height - delta - 5,
                    infoString);
            // 绘制时间信息
            context.drawString(left, height - delta - 5, ZLibrary.Instance()
                    .getCurrentTimeString());
			/*
			 * // draw gauge final int gaugeRight = right - (infoWidth == 0 ? 0
			 * : infoWidth + 10); myGaugeWidth = gaugeRight - left - 2 *
			 * lineWidth;
			 *
			 * context.setLineColor(fgColor); context.setLineWidth(lineWidth);
			 * context.drawLine(left, lineWidth, left, height - lineWidth);
			 * context.drawLine(left, height - lineWidth, gaugeRight, height -
			 * lineWidth); context.drawLine(gaugeRight, height - lineWidth,
			 * gaugeRight, lineWidth); context.drawLine(gaugeRight, lineWidth,
			 * left, lineWidth);
			 *
			 * final int gaugeInternalRight = left + lineWidth + (int)(1.0 *
			 * myGaugeWidth * pagePosition.Current / pagePosition.Total);
			 *
			 * context.setFillColor(fillColor); context.fillRectangle(left + 1,
			 * height - 2 * lineWidth, gaugeInternalRight, lineWidth + 1);
			 *
			 * if (reader.FooterShowTOCMarksOption.getValue()) { if (myTOCMarks
			 * == null) { updateTOCMarks(model); } final int fullLength =
			 * sizeOfFullText(); for (TOCTree tocItem : myTOCMarks) {
			 * TOCTree.Reference reference = tocItem.getReference(); if
			 * (reference != null) { final int refCoord =
			 * sizeOfTextBeforeParagraph(reference.ParagraphIndex); final int
			 * xCoord = left + 2 * lineWidth + (int)(1.0 * myGaugeWidth *
			 * refCoord / fullLength); context.drawLine(xCoord, height -
			 * lineWidth, xCoord, lineWidth); } } }
			 */
        }
		/*
    ...
    private class Header implements HeaderArea {
        ...
        public synchronized void paint(ZLPaintContext context) {
            ...
            context.setFont(reader.FooterFontOption.getValue(), footerAndHeaderSize, false,
                    false, false, false);
            context.setTextColor(fgColor);
            TOCTree currentTOCElement = reader.getCurrentTOCElement();
            if (currentTOCElement != null) {
                String chapterInfo = currentTOCElement.getText();
                if (chapterInfo.length() > 20) {
                    chapterInfo = chapterInfo.substring(0, 20) + "...";
                }
                context.drawString(getLeftMargin(), getHeight() - 5,
                        chapterInfo);
            }
        }
